...
module.exports = (openTasks) => {
  ...
  for(const [task, name] of openTasks){
    ...
    const option = name ? 
      Bits.Option({ text: ":white_check_mark: Mark as completed", value: `complete-task-${task.id}`, emoji: "true"}) :
      Bits.Option({ text: ":man-raising-hand: Take on Task", value: `assign-task-${task.id}`, emoji: "true"})

    tasksInputsArray.push(
      Header({ text: `${task.title}`}),
      Section({type:"mrkdwn", text: `${sectionNameText}\nDue: *${date}*`}).accessory(Elements.OverflowMenu().actionId("blockOpenTaskCheckboxClicked").options([
        option,
        Bits.Option(
          {
            text: ":memo: Edit task",
  ...
  homeTab.blocks(
    Header({ text: `${openTasks.length} open ${pluralize('task', openTasks.length)}` }),
    Divider(),
...
// Section({type:"mrkdwn", text: `Due: *${DateTime.fromJSDate(task.dueDate).toRelativeCalendar()}*`}).accessory(Elements.OverflowMenu().options([
      //   Bits.Option(
      //   {
      ...
      //     }),
      // ])),
      // Actions({blockId: `open-task-status-change-${count}` }).elements(
      //   Elements.Button({ text: 'Complete' }).value('task-complete-btn').actionId('blockOpenTaskCheckboxClicked').primary(true),
      //   Elements.Button({ text: 'Delete' }).value('task-delete-btn').actionId('app-home-nav-completed').danger(true),
      //   Elements.Button({ text: 'Edit' }).value('task-edit-btn').actionId('app-home-nav-create-a-task')
      // ),